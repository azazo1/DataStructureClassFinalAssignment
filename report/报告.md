...
![baseline-score.png](img/baseline-score.png)
...
2. 算法分为两个步骤, 我称之为 Build 和 Search,
    1. Build: 创建不同层次的图, 以不同的频率添加向量到不同的层之中, 添加向量时让其和原先在图中和其最靠近的
       N_LINK 个向量建立连接.
    2. Search:
        1. 先在最稀疏的层中选一个节点开始进行贪婪地查找最接近的向量, 直到图中节点没有邻居比其更加靠近目标向量,
           得到局部最小值节点.
        2. 从上一层的局部最小值节点开始, 在更加密集的下一层中, 重复上述步骤, 直到最底层,
           得到近似的和目标向量最接近的节点.

![hnsw-intro-img.png](hnsw-intro-img.png)
...
### 解决优先级队列比较里的重复计算和重复性检查

要把优先级队列应用下来, 还需要一个步骤: 缓冲每个节点和目标向量的距离.
原因是在优先级队列中, 需要进行多次比较, 若不缓冲距离则会造成大量的重复的运算.

对此我的设计是:
每个 Layer 维护一个独特的 `searchBatch:int`, 用于记录搜索批次, 每当搜索的目标节点或使用的优先级队列发生变化,
此值自增. Layer 中的每个 GraphNode 添加三个字段: `{ batch:int, inQueue:bool, distance:double }`,
分别代表缓冲的距离所属的搜索批次, 是否在队列里和缓冲的距离.

这三个字段不仅可以减少重复的计算, 还能用于[重复性检查](#优先级队列特殊情况的解决思路),
...
最后放进优先级队列里的是 GraphNode 的指针, 同时传入一个比较函数给优先级队列, 用于取得 distance 并比较.
...
2. 如果只传入索引, 那么在比较时获取 distance 的值就需要一个 Layer 的引用, 于是在比较函数中就需要拥有
   Layer 的指针, 而获取 Layer 的引用比较困难, 会使代码的耦合性变高.
...
- 每次使用节点的缓冲距离时, 需要提前计算.
- 每次从优先级队列中放入值前需要设置 inQueue = true.
- 每次从优先级队列中弹出值时需要设置 inQueue = false.

步骤为:

1. 如果 Layer 的节点数量小于等于 k, 直接把 Layer 中所有的节点索引返回.
2. 从 Layer 中的前 k 个节点开始, 把这些节点放入优先级队列(pq: `PriorityQueue<GraphNode *>`,
   堆顶存放着距离最远的节点).
...
最终在测试集 8000 上的总搜索速度可以到 1 秒左右(N_LINK 为 3), 但准确率不尽人意.
...
最终找到了此项目中的 [knn 搜索方法: searchBaseLayerST](../reference/hnswlib/hnswalg.h),
把此方法的几个分支简化(假定 `bare_bone_search` 为 true)后, 发现方法在搜索时使用了两个优先级队列,
一个优先级队列是正序的, 另一个是倒序的.
...
3. 循环, 直到 cq 为空:
    ...
    3. 遍历所有 nn 的没被探索过的邻居 ng: (被加入到 rq 或者 cq 过则说明被探索过)
        1. 如果 ng 的距离比下界小则把 ng 加入 cq 和 rq.
        2. 删除 rq 中的距离最远的几个节点, 直到 rq 尺寸小于等于 k.
4. 取出 rq 的节点作为返回值 (循环中已经保证其尺寸小于等于 k).

此算法相比我自己想的优先级队列搜索, 令我为之震惊的点在于其使用两个优先级队列,
每次循环弹出队列中最近的节点进行探索而不是最远的节点, 还有其使用下界进行判断的方式也令我大开眼界.

我将这个算法和结合 [searchNearestNode](../src/hnsw/Layer.cpp) 使用, 形成了 HNSW 算法 KNN 搜索部分.

## 结果

![knsw-knn-score.png](img/hnsw-knn-score.png)
...
![hnsw-testdataset-profile.png](img/hnsw-testdataset-profile.png)
