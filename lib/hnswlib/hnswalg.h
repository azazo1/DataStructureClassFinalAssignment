...
namespace hnswlib {
    ...
    public:
        ...
        searchBaseLayer(tableint ep_id, const void *data_point, int layer) {
            ...
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                tableint> >, CompareByFirst> top_candidates;
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                tableint> >, CompareByFirst> candidateSet;
        ...
        searchBaseLayerST(
            ...
            // priority_queue top 返回`最大的`值
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                tableint> >, CompareByFirst> top_candidates;
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                tableint> >, CompareByFirst> candidate_set; // top 返回距离最短的节点

            dist_t lowerBound;
            constexpr bool bare_bone_search = true; // 假定
            if (bare_bone_search ||
                (!isMarkedDeleted(ep_id) && ((!isIdAllowed) || (*isIdAllowed)(
                                                 getExternalLabel(ep_id))))) {
                ...
                candidate_set.emplace(-dist, ep_id); // 注意 candidate_set 是负的距离
            } else {
                lowerBound = std::numeric_limits<dist_t>::max();
            ...
            while (!candidate_set.empty()) {
                ...
                for (size_t j = 1; j <= size; j++) {
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        if (!bare_bone_search && stop_condition) {
                            flag_consider_candidate = stop_condition->should_consider_candidate(
                                dist, lowerBound);
                        } else {
                            // 如果 top_candidate 数量太少, 直接加入 top_candidate 和 candidate_set.
                            // 否则只有邻居的距离比下界小才能加入 top_candidate 和 candidate_set.
        ...
        linklistsizeint *get_linklist(tableint internal_id, int level) const {
            return (linklistsizeint *) (linkLists_[internal_id] + (level - 1) *
        ...
        */
        ...
        void updatePoint(const void *dataPoint, tableint internalId,
            ...
            for (int layer = 0; layer <= elemLevel; layer++) {
                ...
                for (auto &&neigh: sNeigh) {
                    ...
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                        tableint> >, CompareByFirst> candidates;
                    size_t size =
        ...
        void repairConnectionsForUpdate(
            ...
            for (int level = dataPointLevel; level >= 0; level--) {
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                    tableint> >, CompareByFirst> topCandidates = searchBaseLayer(
                    currObj, dataPoint, level);

                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                    tableint> >, CompareByFirst> filteredTopCandidates;
                    ...
                    if (epDeleted) {
                        filteredTopCandidates.emplace(
                            fstdistfunc_(dataPoint, getDataByInternalId(entryPointInternalId),
                                         dist_func_param_), entryPointInternalId);
        ...
        tableint addPoint(const void *data_point, labeltype label, int level) {
            ...
            if ((signed) currObj != -1) {
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                    ...
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                        tableint> >, CompareByFirst> top_candidates = searchBaseLayer(
                        currObj, data_point, level);
                    if (epDeleted) {
                        top_candidates.emplace(
                            fstdistfunc_(data_point, getDataByInternalId(enterpoint_copy),
                                         dist_func_param_), enterpoint_copy);
                        if (top_candidates.size() > ef_construction_)
                            top_candidates.pop();
                    }
        ...
        searchStopConditionClosest(
            ...
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t,
                tableint> >, CompareByFirst> top_candidates;
            top_candidates = searchBaseLayerST<false>(currObj, query_data, 0, isIdAllowed,
                                                      &stop_condition);
